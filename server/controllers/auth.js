...
const sanitizeUser = (user, ctx) => {
  ...
  const userSchema = strapi.getModel(`plugin::${pluginId}.user`);
...
module.exports = {
  async callback(ctx) {
    ...
    const store = strapi.store({ type: "plugin", name: pluginId });
    ...
    if (provider === "local") {
      ...
      const user = await strapi.query(`plugin::${pluginId}.user`).findOne({
        where: {
          provider,
  ...
  async register(ctx) {
    ...
    const { register } = strapi.config.get(`plugin.${pluginId}`);
    const alwaysAllowedKeys = ["username", "password", "email"];
    const userModel = strapi.contentTypes[`plugin::${pluginId}.user`];
