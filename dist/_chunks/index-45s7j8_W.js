...
const EmailTemplatesPage = () => {
  ...
  const { isLoading: isLoadingData, data } = reactQuery.useQuery(
    [index.pluginId, "email-templates"],
    async () => {
      const { data: data2 } = await get(`/${index.pluginId}/email-templates`);
      return data2;
    },
    {
      onSuccess() {
  ...
  const submitMutation = reactQuery.useMutation(
    (body) => put(`/${index.pluginId}/email-templates`, { "email-templates": body }),
    {
      async onSuccess() {
        await queryClient.invalidateQueries([index.pluginId, "email-templates"]);
        toggleNotification({
          type: "success",
          message: {
...
//# sourceMappingURL=index-45s7j8_W.js.map
