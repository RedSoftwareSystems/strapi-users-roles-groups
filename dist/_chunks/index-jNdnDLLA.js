...
const AdvancedSettingsPage = () => {
  ...
  const { isLoading: isLoadingData, data } = reactQuery.useQuery(
    [index.pluginId, "advanced"],
    async () => {
      const { data: data2 } = await get(`/${index.pluginId}/advanced`);
      return data2;
    },
    {
      onSuccess() {
  ...
  const submitMutation = reactQuery.useMutation(
    (body) => put(`/${index.pluginId}/advanced`, body),
    {
      async onSuccess() {
        await queryClient.invalidateQueries([index.pluginId, "advanced"]);
        toggleNotification({
          type: "success",
          message: {
...
//# sourceMappingURL=index-jNdnDLLA.js.map
