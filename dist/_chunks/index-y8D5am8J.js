...
const ProvidersPage = () => {
  ...
  const { isLoading: isLoadingData, data } = reactQuery.useQuery(
    [index.pluginId, "get-providers"],
    async () => {
      const { data: data2 } = await get(`/${index.pluginId}/providers`);
      return data2;
    },
    {
      initialData: {}
  ...
  const submitMutation = reactQuery.useMutation(
    (body) => put(`/${index.pluginId}/providers`, body),
    {
      async onSuccess() {
        await queryClient.invalidateQueries([index.pluginId, "providers"]);
        toggleNotification({
          type: "success",
...
//# sourceMappingURL=index-y8D5am8J.js.map
