...
const render = () => ({
  ...renderRTL(
    <Route path={`/settings/${pluginId}/roles/:id`} component={EditPage} />,
    {
      wrapper({ children }) {
            ...
            <ThemeProvider theme={lightTheme}>
              <QueryClientProvider client={client}>
                <NotificationsProvider>
                  <MemoryRouter
                    initialEntries={[`/settings/${pluginId}/roles/1`]}
                  >
...
describe("Roles – EditPage", () => {
  ...
  it("will show an error if the user does not fill out the description field", async () => {
    ...
    expect(getByRole("textbox", { name: "Description *" })).toHaveAttribute(
      "aria-invalid",
      "true"
    );
  });
  ...
  it("will update the Advanced Settings panel when you click on the cog icon of a specific permission", async () => {
    ...
    expect(
      getByRole("heading", { name: "Bound route to address .create" })
    ).toBeInTheDocument();
